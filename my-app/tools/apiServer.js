...
server.post("/fileserver/", function(req, res, next) {
  const error = validateUserDetails(req.body);
  if (error) {
    res.status(400).send(error);
  } else {
    req.body.slug = createSlug(req.body.title); // Generate a slug for new user profiles.
    next();
  }
});
...
function validateUserDetails(userDetails) {
  if (!userDetails.name) return "Name is required.";
  if (!userDetails.gender) return "Gender is required.";
  return "";
}
